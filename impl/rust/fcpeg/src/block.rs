...
macro_rules! rule_id {
    ($cross_refered_alias_names:expr, $file_alias_name:expr, $block_name:expr, $rule_name:expr) => {
        {
            let replaced_file_alias_name = if !$cross_refered_alias_names.contains(&$file_alias_name) {
                &$file_alias_name
            } else {
                ""
            };

            format!("{}.{}.{}", replaced_file_alias_name, $block_name, $rule_name)
        }
    };
}
...
pub struct BlockParser {
    ...
    cross_refered_alias_names: Arc<Vec<String>>,
...
impl BlockParser {
    ...
    pub fn get_rule_map(cons: Rc<RefCell<Console>>, fcpeg_file_map: &mut FCPEGFileMap, enable_memoization: bool) -> ConsoleResult<Arc<Box<RuleMap>>> {
        ...
        for (file_alias_name, fcpeg_file) in fcpeg_file_map.iter() {
            let mut block_parser = BlockParser {
                ...
                cross_refered_alias_names: fcpeg_file_map.cross_refered_alias_names.clone(),
                    ...
                    match start_cmd.clone() {
                        BlockCommand::Start { pos, file_alias_name, block_name, rule_name } => {
                            ...
                            if self.file_alias_name == "" {
                                ...
                                let rule_id = rule_id!(self.cross_refered_alias_names, file_alias_name, block_name, rule_name);
    ...
    fn to_define_cmd(&mut self, cmd_node: &SyntaxNode) -> ConsoleResult<BlockCommand> {
        ...
        let rule_id = rule_id!(self.cross_refered_alias_names, self.file_alias_name, self.block_name, rule_name);
    ...
    fn to_start_cmd(&mut self, cmd_node: &SyntaxNode) -> ConsoleResult<BlockCommand> {
        ...
        if DEFAULT_START_RULE_ID == rule_id!(self.cross_refered_alias_names, file_alias_name, block_name, rule_name) {
                ...
                match name.as_str() {
                    ...
                    ".Rule.Generics" | ".Rule.Func" => {
                        ...
                        let id = if name == ".Rule.Func" && PRIM_FUNC_NAMES.contains(&joined_raw_id.as_str()) {
                            ...
                            match BlockParser::to_rule_id(&self.cons, &pos, &raw_id, &self.block_alias_map, &self.file_alias_name, &self.block_name, &self.cross_refered_alias_names) {
                                Ok(id) => {
                    ...
                    ".Rule.ID" => {
                        ...
                        let id = match BlockParser::to_rule_id(&self.cons, &pos, &BlockParser::to_string_vec(&self.cons, chain_id_node)?, &self.block_alias_map, &self.file_alias_name, &self.block_name, &self.cross_refered_alias_names) {
                            Ok(id) => {
                                if !self.used_rule_ids.contains_key(&id) {
                                    self.used_rule_ids.insert(id.clone(), pos.clone());
                                }
    ...
    fn to_rule_id(cons: &Rc<RefCell<Console>>, pos: &CharacterPosition, id_tokens: &Vec<String>, block_alias_map: &HashMap<String, String>, file_alias_name: &String, block_name: &String, cross_refered_alias_names: &Arc<Vec<String>>) -> ConsoleResult<String> {
        let (new_id, id_block_name, id_rule_name) = match id_tokens.len() {
            1 => {
                let id_rule_name = id_tokens.get(0).unwrap();
                let new_id = rule_id!(cross_refered_alias_names, file_alias_name, block_name, id_rule_name);
            ...
            3 => {
                ...
                let new_id = rule_id!(cross_refered_alias_names, file_alias_name, block_name, rule_name);
